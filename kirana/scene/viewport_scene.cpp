...
void kirana::scene::ViewportScene::onSceneLoaded()
{
    if (m_currentScene.isInitialized())
    {
        ...
        for (uint32_t i = 0; i < sceneObjects.size(); i++)
        {
            ...
            m_objectIndexTable[name] = static_cast<uint32_t>(m_sceneRenderables.size());
        ...
        {
            m_sceneInfo.numVertices += static_cast<uint32_t>(m->getVertices().size());
            m_sceneInfo.numIndices += static_cast<uint32_t>(m->getIndices().size());
        }
