...
inout vec3 lightDirection, inout float pdf)
{
    vec3 ffnormal = dot(viewDirection, hitpoint.normal) < 0.0 ? - hitpoint.normal : hitpoint.normal;
    vec3 randomDir = normalize(randomHemispherical(payload.seed, vec3[3](hitpoint.tangent, hitpoint.bitangent, hitpoint.normal)));
...
void main()
{
    ...
    float pdf = 1.0;
    vec3 brdf = principledBRDF(matData, hitpoint, viewDirection, reflectedDirection, pdf);

    payload.ray = Ray(rayOrigin, reflectedDirection);
    payload.color = matData.emissiveColor.rgb * matData.emissiveIntensity; // Light emmitted
    payload.weight = brdf * abs(dot(hitpoint.normal, reflectedDirection)) / pdf;
}